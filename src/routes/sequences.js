...
router.get('/', rbac('sequences', 'read'), async (req, res) => {
  try {
    const list = await ss.list(req.companyId);
...
router.post('/', rbac('sequences', 'write'), async (req, res) => {
  try {
    const s = await ss.create(req.companyId, req.user._id, req.body);
...
router.get('/:id', rbac('sequences', 'read'), async (req, res) => {
  try {
    res.json(await ss.findById(req.params.id, req.companyId));
  } catch (e) {
...
router.patch('/:id', rbac('sequences', 'write'), async (req, res) => {
  try {
    res.json(await ss.update(req.params.id, req.companyId, req.body));
  } catch (e) {
    res.status(400).json({ error: e.message });
  }
});
...
router.delete('/:id', rbac('sequences', 'delete'), async (req, res) => {
  try {
    res.json(await ss.remove(req.params.id));
  } catch (e) {
    res.status(400).json({ error: e.message });
  }
});
