...
async function startShipBobLogisticsAdapter() {
  try {
    ...
    serviceRunner.start('shipbob-inventory-changes', async () => {
      try {
    ...
    serviceRunner.start('shipbob-order-created', async () => {
      try {
        if (kafkaConnected && producer) {
    ...
    serviceRunner.start('shipbob-inventory-generate', async () => {
      try {
        const inventoryChange = generateMockInventoryChange();
    ...
    serviceRunner.start('shipbob-order-generate', async () => {
      try {
        const orderCreated = generateMockOrderCreated();
